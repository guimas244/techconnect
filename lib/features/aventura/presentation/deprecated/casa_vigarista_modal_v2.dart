...
class _CasaVigaristaModalV2State extends State<CasaVigaristaModalV2>
  ...
  int get custoAposta =>
      2 * (_historiaAtual.tier >= 11 ? 2 : _historiaAtual.tier);
  int get custoCura =>
      1 * (_historiaAtual.tier >= 11 ? 2 : _historiaAtual.tier);
  int get custoFeirao =>
      ((_historiaAtual.tier >= 11 ? 2 : _historiaAtual.tier) * 1.5).ceil();
    ...
    return Container(
          ...
          decoration: const BoxDecoration(
            gradient: LinearGradient(
              ...
              colors: [Color(0xFF1B1B2F), Color(0xFF16213E), Color(0xFF0F3460)],
            ),
          ),
          child: Stack(
            children: [
              LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  final safeWidth = width <= 0 ? size.width : width;
                  final safeHeight = height <= 0 ? size.height : height;
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            Expanded(child: SingleChildScrollView(child: _buildShopCarousel())),
  ...
  Widget _buildAnimatedHeader() {
    return GlassmorphicContainer(
          ...
          borderGradient: LinearGradient(
            colors: [const Color(0xFF6B6B6B), const Color(0xFF8B8B8B)],
          ),
          child: Stack(
            children: [
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(
                      width: 30,
                    ), // Espaço para manter o layout simétrico
                  ],
                ),
              ),
            ],
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      ).animate().slideX(delay: 800.ms, duration: 400.ms).fadeIn(),
                    ],
                  ),
                ),
              ],
    ...
    return GestureDetector(
      ...
      child: GlassmorphicContainer(
        ...
        linearGradient: LinearGradient(
          ...
          colors:
              podeComprar
                  ? [colors[0].withOpacity(0.3), colors[1].withOpacity(0.2)]
                  : [
        ...
        borderGradient: LinearGradient(
          colors:
              podeComprar
                  ? [colors[0].withOpacity(0.8), colors[1].withOpacity(0.6)]
                  : [
                      ...
                      children: [
                        ...
                        AutoSizeText(
                          ...
                          style: GoogleFonts.philosopher(
                            fontSize: 10,
                            color:
                                podeComprar ? colors[0] : Colors.grey.shade600,
                            fontStyle: FontStyle.italic,
                          ),
                          textAlign: TextAlign.center,
                          maxLines: 1,
      ...
      builder: (BuildContext context) {
        return Dialog(
              ...
              child: GlassmorphicContainer(
                ...
                borderGradient: LinearGradient(
                  colors: [const Color(0xFF6A0DAD), const Color(0xFF8B008B)],
                ),
                child: Padding(
                  padding: const EdgeInsets.all(30),
                  child: Column(
                      ...
                      Row(
                        children: [
                          ...
                          Flexible(
                            child: GestureDetector(
                              onTap:
                                  _historiaAtual.score >= custoFeirao
                                      ? () {
                                        Navigator.of(context).pop();
                                        _abrirBiblioteca();
                                      }
      ...
      builder: (BuildContext context) {
        return Dialog(
              ...
              child: GlassmorphicContainer(
                ...
                borderGradient: LinearGradient(
                  colors: [const Color(0xFFFF8C00), const Color(0xFFFFD700)],
                ),
                child: Padding(
                  padding: const EdgeInsets.all(30),
                  child: Column(
  ...
  void _apostarItem() async {
    ...
    late HistoriaJogador historiaPosCompra;
    var historiaPosCompraDefinida = false;
    HistoriaJogador? historiaResultado;

    try {
      historiaPosCompra = _historiaAtual.copyWith(
        score: _historiaAtual.score - custoAposta,
      );
      historiaPosCompraDefinida = true;
      ...
      await widget.onHistoriaAtualizada(historiaPosCompra);

      if (!mounted) return;

      // Aguarda um curto intervalo para garantir que a árvore estabilize antes de abrir o modal
      await Future.delayed(const Duration(milliseconds: 200));

      if (!mounted) return;

      final historiaDialogo = historiaPosCompra;

      await showDialog(
        context: navigatorContext,
        barrierDismissible: false,
        builder:
            (dialogContext) => ModalItemObtido(
              item: item,
              monstrosDisponiveis: historiaDialogo.monstros,
              onEquiparItem: (monstro, itemObtido) async {
                final monstrosAtualizados =
                    historiaDialogo.monstros.map((m) {
                ...
                final historiaAtualizadaLocal = historiaDialogo.copyWith(
                  monstros: monstrosAtualizados,
                );
                historiaResultado = historiaAtualizadaLocal;
                await widget.onHistoriaAtualizada(historiaAtualizadaLocal);
      ...
      if (mounted) {
        final historiaParaAplicar =
            historiaResultado ??
            (historiaPosCompraDefinida ? historiaPosCompra : null);

        setState(() {
          if (historiaParaAplicar != null) {
            _historiaAtual = historiaParaAplicar;
          }
          _comprando = false;
        });
      }
  ...
  void _apostarMagia() async {
    ...
    try {
      ...
      await showDialog(
        ...
        barrierDismissible: false,
        builder:
            (dialogContext) => ModalMagiaObtida(
              magia: magia,
              monstrosDisponiveis: historiaDialogo.monstros,
              onEquiparMagia: (
                monstro,
                magiaObtida,
                habilidadeSubstituida,
              ) async {
                ...
                final monstrosAtualizados =
                    historiaDialogo.monstros.map((m) {
                      if (m.tipo == monstro.tipo && m.level == monstro.level) {
                        final habilidadesAtualizadas =
                            m.habilidades.map((h) {
                              if (h.nome == habilidadeSubstituida.nome) {
  ...
  void _apostarCura() async {
    ...
    try {
      ...
      // 4. Captura o context raiz ANTES de qualquer operação assíncrona
      final navigatorContext =
          Navigator.of(context, rootNavigator: true).context;
      ...
      historiaResultado = await _mostrarResultadoCura(
        porcentagemCura,
        historiaPosCompra,
        navigatorContext,
      );
  ...
  Habilidade _gerarHabilidadeAleatoria() {
    ...
    final habilidades = GeradorHabilidades.gerarHabilidadesMonstro(
      Tipo.normal,
      null,
      levelCustomizado: tierAtual,
    );
    ...
    if (habilidades.isNotEmpty) {
      ...
      return Habilidade(
        ...
        descricao:
            '${habilidadeBase.descricao} (obtida na Biblioteca do Vigarista)',
  ...
  void _mostrarResultadoItem(Item item, HistoriaJogador historia) {
    showDialog(
      ...
      barrierDismissible: false,
      builder:
          (context) => ModalItemObtido(
            item: item,
            monstrosDisponiveis: historia.monstros,
            onEquiparItem: (monstro, itemObtido) async {
              final monstrosAtualizados =
                  historia.monstros.map((m) {
                    if (m.tipo == monstro.tipo && m.level == monstro.level) {
                      return m.copyWith(itemEquipado: itemObtido);
                    }
                    return m;
              ...
              final historiaFinal = historia.copyWith(
                monstros: monstrosAtualizados,
              );
              ...
              _mostrarMensagemSucesso(
                'Item ${itemObtido.nome} equipado em ${monstro.tipo.displayName}!',
              );
            },
          ),
    );
          ...
          (context) => ModalMagiaObtida(
            ...
            onEquiparMagia: (
              ...
              final monstrosAtualizados =
                  historia.monstros.map((m) {
                    if (m.tipo == monstro.tipo && m.level == monstro.level) {
                      final novasHabilidades =
                          m.habilidades.map((h) {
                            return h == habilidadeSubstituida
                                ? novaHabilidade
                                : h;
                          }).toList();
              ...
              _mostrarMensagemSucesso(
                '${monstro.tipo.displayName} aprendeu ${novaHabilidade.nome}!',
              );
            },
          ),
    );
  ...
  Future<HistoriaJogador?> _mostrarResultadoCura(
    int porcentagem,
    HistoriaJogador historia,
    BuildContext navigatorContext,
  ) async {
    HistoriaJogador? historiaFinal;

    await showDialog(
      context: navigatorContext,
      barrierDismissible: false,
      builder:
          (dialogContext) => ModalCuraObtida(
            porcentagem: porcentagem,
            monstrosDisponiveis: historia.monstros,
            onCurarMonstro: (monstro, porcentagemCura) async {
              final curaTotal = (monstro.vida * porcentagemCura / 100).round();
              final novaVidaAtual = (monstro.vidaAtual + curaTotal).clamp(
                0,
                monstro.vida,
              );
              ...
              final novaHistoria = historia.copyWith(
                monstros: monstrosAtualizados,
              );
              historiaFinal = novaHistoria;

              await widget.onHistoriaAtualizada(novaHistoria);

              if (mounted) {
                Navigator.of(dialogContext).pop();
                _mostrarMensagemSucesso(
                  '${monstro.tipo.displayName} foi curado em $porcentagemCura%!',
                );
              }
            },
          ),
  ...
  void _mostrarModalBiblioteca(
    List<Habilidade> magias,
    HistoriaJogador historia,
  ) {
    showDialog(
      context: context,
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            // Nome do item
                            Expanded(child: _buildScrollingText(item.nome)),

                            // Botão de comprar
                            GestureDetector(
                              onTap:
                                  podeComprar
                                      ? () => _comprarItemFeirao(item, historia)
                                      : null,
                              child: Container(
                                padding: const EdgeInsets.symmetric(
                                  horizontal: 8,
        ...
        child: FadeInAnimation(
          child: Container(
            ...
            child: GlassmorphicContainer(
              ...
              borderGradient: LinearGradient(colors: [cores[0], cores[1]]),
              child: Padding(
                padding: const EdgeInsets.all(12),
                child: Row(
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.circular(12),
                        gradient: LinearGradient(colors: [cores[0], cores[1]]),
                        boxShadow: [
                          BoxShadow(
